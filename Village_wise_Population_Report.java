...
public class Village_wise_Population_Report extends JFrame implements ActionListener
{
	...
	ResultSet rs;
	int c;
	String []colname={"Village Code","Village Name","Population Code","Number of Male Population","Number of Female Population","Number of Child Population"};
...
public Village_wise_Population_Report()
{
		...
		lbl=new JLabel("Report of Village wise Population                                                 ");
		...
		rs=myStatement.executeQuery("select * from population_master");
		...
		{
		ResultSet rs1;
		rs1=myStatement.executeQuery("select v.Village_code,v.village_name,pp.population_code,pp.no_of_male_population,pp.no_of_female_population,pp.no_of_child_population from village_master v,Population_master pp where v.village_code=pp.village_code  order by v.village_code");
		for(int i=0;i<c;i++)
		{
		rs1.next();
