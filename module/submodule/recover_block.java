...
public class recover_block extends JFrame
{
	...
	Statement stat;
	ResultSet rs;
	private String str1,str2,str3,str4,str5,str6,str7,str8,str9,tmp;
	String colhead[]={"District_code","Subdivision_code","Block_code","Block_name","estd","Bdo_name","User_Name","Status","Date Of Modification"};
	Object data[][]=new Object[30][9];
	JTable table=new JTable(data,colhead);
	Cursor cr;
	
	public recover_block()
	{
		...
		title=new JLabel("Recover Block Information");
		...
		load();
         ...
         public void load()
         {
            ...
            try
            {
              ...
              rs=stat.executeQuery("select District_code,subdiv_code,block_code,block_name,TO_CHAR(estd,'DD-MON-YYYY'),Bdo_name,user_name,status,TO_CHAR(dt,'DD-MON-YYYY') from bkp_block order by district_code");
              while(rs.next())
              {
                str1=rs.getString(1);
                str2=rs.getString(2);
   	...
   	private class okListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			try
			{
				  str1=t1.getText();
				  tmp="update";
				  rs=stat.executeQuery("select district_code,subdiv_code,block_name,TO_CHAR(estd,'DD-MON-YYYY'),Bdo_name,status from bkp_block where block_code='"+str1.trim()+"'");
                 rs.next();
                str2=rs.getString(1);
                str3=rs.getString(2);
                str4=rs.getString(3);
                ...
                str6=rs.getString(5);
                str7=rs.getString(6);
                if(tmp.equals(str7.trim()))
                {
                	if(str5==null)
                	str5="";
                stat.executeUpdate("update BLOCK_master set district_code='"+str2+"',subdiv_CODE='"+str3+"',block_name='"+str4.trim()+"',estd='"+str5+"',bdo_name='"+str6.trim()+"' where block_code='"+str1.trim()+"'");
				stat.executeUpdate("delete from bkp_block where block_code='"+str1.trim()+"'");
				stat.executeUpdate("commit");
