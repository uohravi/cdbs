...
public class recover_subdivision extends JFrame
{
	...
	String colhead[]={"District_code","Subdivision_code","Subdivision_name","estd","Sdo_name","User_Name","Status","Date Of Modification"};
	Object data[][]=new Object[30][8];
	JTable table=new JTable(data,colhead);
	Cursor cr;
	
	public recover_subdivision()
	{
		...
		title=new JLabel("Recover Subdivision Information");
    	...
    	l1=new JLabel("Enter Subdivision_Code");
		...
		load();
         ...
         public void load()
         {
            ...
            try
            {
              ...
              rs=stat.executeQuery("select District_code,subdiv_code,subdiv_name,TO_CHAR(estd,'DD-MON-YYYY'),sdo_name,user_name,status,TO_CHAR(dt,'DD-MON-YYYY') from bkp_subdivision order by district_code");
              while(rs.next())
              {
                str1=rs.getString(1);
                str2=rs.getString(2);
   	...
   	private class okListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			try
			{
				  str1=t1.getText();
				  tmp="update";
				  rs=stat.executeQuery("select district_code,subdiv_name,TO_CHAR(estd,'DD-MON-YYYY'),sdo_name,status from bkp_subdivision where subdiv_code='"+str1.trim()+"'");
                 rs.next();
                str2=rs.getString(1);
                str3=rs.getString(2);
                str4=rs.getString(3);
                ...
                if(tmp.equals(str6.trim()))
                {
                	if(str4==null)
                	str4="";
				stat.executeUpdate("update subdivision_master set district_code='"+str2+"',subdiv_name='"+str3.trim()+"',estd='"+str4+"',sdo_name='"+str5.trim()+"' where subdiv_code='"+str1.trim()+"'");
				stat.executeUpdate("delete from bkp_subdivision where subdiv_code='"+str1.trim()+"'");
				stat.executeUpdate("commit");
